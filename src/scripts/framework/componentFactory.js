...
export default class ComponentFactory extends BoosterFactory {
  ...
  lazyload(el) {
    ...
    const selector = el.getAttribute('id') ? '#' + el.getAttribute('id') : null;
    if (selector === null) {
      return console.warn(
        `Booster Pack: an instance of ${component} doesn't have an ID attribute. Skipping.`,
      );
    }
  ...
  importLocal(component, selector, promises) {
    Promise.all(promises).then(() => {
      ...
      import(`../components/local/${component}.js`).then((lazyComponent) => {
  ...
  importVue(el, component, selector, promises) {
    ...
    Promise.all(promises).then(() => {
      ...
      import(`../components/vue/${component}.vue`).then((vueComponent) => {
